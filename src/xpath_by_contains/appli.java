package xpath_by_contains;

import org.openqa.selenium.By;
import org.openqa.selenium.WebDriver;
import org.openqa.selenium.chrome.ChromeDriver;

public class appli {
		public static void main(String[] args) throws Throwable {
			System.setProperty("webdriver.chrome.driver","C:\\Users\\shree\\Downloads\\chromedriver_win32\\chromedriver.exe");
			WebDriver driver=new ChromeDriver();
			driver.get("https://demo.applitools.com/");
			//Enter the UN
			driver.findElement(By.xpath("//input[contains(@id,'username')]")).sendKeys("Komal");
			driver.findElement(By.xpath("//input[contains(@id,'password')]")).sendKeys("18061998");
			driver.findElement(By.xpath("//a[contains(@id,'log-in')]")).click();
			
			//verify home page is open or not
			String expT="ACME demo app";
			Thread.sleep(5000);
			String actT=driver.getTitle();
			if(actT.equals(expT)) {
				System.out.println("home page is opened and TC is pass");
			}
			else {
				System.out.println("home page is not opened and TC is fail");
	
			}
			}

}
